...
| NPC voice lines | **Yes** — Ollama (prompt from `Assets/Prompts/cap_personality.j2`) | Dialogue panel LLM block; quest facts are authored separately |
| AI side quest text | **Yes** — Ollama JSON (`AiQuestPlanner`); objectives validated in C# | Cap dialogue when offered; HUD uses C# hints |
...
3. **Options:** **AI-driven dialogue (Ollama)** (`GameSettings.LlmEnabled`) and **Fast AI responses** (`GameSettings.LlmFastMode`). When LLM is off:
   ...
   - **Another line** is hidden; zone flavor and Ollama planners are skipped (fallback side quests still register).
