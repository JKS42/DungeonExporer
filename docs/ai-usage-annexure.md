...
- Prompt engineering and iterations (`docs/prompts-used.md`, `Assets/Prompts/cap_personality.j2`).
- Local LLM gameplay integration and testing (`Assets/Scripts/OllamaHandler.cs`, `DialoguePanelController.cs`, `CharacterPersonalityTemplateManager.cs`).
...
- Improve response quality and reliability through prompt/template iteration (C# Cap template + fast mode).
...
- Most recent documented AI-assisted updates: 2026-06-18 (`docs/refinements-changes.md` — C# Cap template, Ollama queue, fast mode, gameplay text, doc sync).
...
- `Assets/Prompts/cap_personality.j2` and `Assets/Prompts/cap_context.json` (runtime prompt artifacts).
- `prompts/cap_personality.jinja2` (legacy offline comparison).
