...
DialoguePanelController
   ...
   ├─► Ask Cap: reactive RequestChat (/api/chat) with player question
   └─► Another line: invalidate cache + re-fetch (RequestGeneration)
...
**Runtime:** `CapPersonalityPromptBuilder` → `python prompts/render_cap_prompt.py` (Jinja2) → user prompt string → `OllamaHandler.RequestGeneration` (voice / **Another line**) or `RequestChat` (**Ask Cap**).
...
- When `false`, opening Cap shows an authored canned line; **Ask Cap** and **Another line** are hidden; **DungeonFlavorNarrator** skips requests. Level-load trap/content planners also skip Ollama (procedural fill only). Still TODO: centralize in `OllamaHandler` for the debug test UI.
