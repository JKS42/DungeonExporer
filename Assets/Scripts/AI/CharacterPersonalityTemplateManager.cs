...
{
    /// <summary>
    /// Loads <c>Assets/Prompts/cap_personality.j2</c> and renders <c>{{ field }}</c> placeholders.
    /// Adapted from DatingSim <c>CharacterPersonalityTemplateManager</c> — no Python/Jinja2 at runtime.
    /// </summary>
    public class CharacterPersonalityTemplateManager : MonoBehaviour
    {
        ...
        private static CharacterPersonalityTemplateManager _instance;

        /// <summary>Cap context fields — mirrors <c>cap_context.json</c> and <c>cap_personality.j2</c>.</summary>
        [Serializable]
        public class CapContextData
        {
            ...
            public string background =
                ...
                "He lost count of the seasons after his third favourite tea mug cracked on a trap tile.";
            public string personality_traits =
                ...
                "- Lighthearted only — no horror, grimdark, or fourth-wall breaks";
            public string communication_style =
                ...
                "Uses short vivid images (damp moss, brass keys, humming crimson floors). Encouraging but never condescending.";
            ...
            public string interaction_scenario =
                "The adventurer has just found Cap in a mint-tinted safe room near the maze spawn. Torchlight flickers on old maps pinned to the wall.";
            public string additional_context =
                "Cap knows the maze layout in broad strokes but will not spoil secrets. He nudges, never commands. " +
                "He may joke about his whiskers, stale tea, or overconfident trainees who ignored spike strips.";
            ...
            public string location = "Cap's hub safe room near the maze spawn — maps, kettle steam, and a worn armchair.";
        }
        ...
        /// <summary>Load the template and render it with Cap context (DatingSim-style instance API).</summary>
        public string RenderCapPersonality(CapContextData characterData)
        {
            if (characterData == null)
                return string.Empty;

            if (!TryLoadTemplate(ResolveTemplateFileName(), out string templateContent))
                return string.Empty;

            return RenderTemplate(templateContent, characterData);
        }

        /// <summary>Full system prompt for Cap dialogue.</summary>
        public string GetCapSystemPrompt(CapContextData characterData) => RenderCapPersonality(characterData);

        public static string RenderCapPersonality(CapContextData context, string templateFileName = DefaultTemplateFileName)
        {
            if (context == null)
                return string.Empty;

            if (!TryLoadTemplate(templateFileName, out string templateContent))
                return string.Empty;
        ...
        public static string RenderTemplate<T>(string template, T data) where T : class
        {
        ...
        public static bool TryResolvePromptsPath(string fileName, out string fullPath)
        {
            ...
            string repoPrompts = Path.GetFullPath(Path.Combine(Application.dataPath, "..", "prompts", fileName));
            if (File.Exists(repoPrompts))
            {
                fullPath = repoPrompts;
                return true;
            }
        ...
        public static CapContextData BuildCapContext(
            ...
            bool reactive)
        {
            CapContextData defaults = null;
            if (!TryLoadDefaultContext(out defaults))
                defaults = new CapContextData();

            var data = new CapContextData
            {
